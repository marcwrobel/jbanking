...
/**
 * A partial compiled representation of a SWIFT expression (a kind of regular expression) as used in
 * many SWIFT documents (for instance <a
 * href="http://www.swift.com/dsp/resources/documents/IBAN_Registry.pdf">the IBAN registry
 * document</a>).
 *
 * <p>This class internally uses a {@link java.util.regex.Pattern} by transforming the SWIFT
 * expression to a traditional regular expression. As a result the SwiftPattern API is similar to
 * the {@link java.util.regex.Pattern Pattern API}.
 *
 ...
 *   <li>{@code 4!a3c} (corresponding regex [A-Za-z0-9]{4}[A-Z]{1,3}) : four consecutive upper or
 *       lower case alphanumeric characters followed by one to three upper case letters
 *   <li>{@code 2e4!a} (corresponding regex [ ]{1,2}) : one or two consecutive spaces followed by

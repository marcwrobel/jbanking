...
/**
 * Thrown to indicate that an attempt has been made to convert a string to a {@link
 * fr.marcwrobel.jbanking.creditor.CreditorIdentifier}, but that the string does not have the
 * appropriate format.
 *
...
public class CreditorIdentifierFormatException extends RuntimeException {
  ...
  /**
   * Constructs a <code>CreditorIdentifierFormatException</code> with the string that caused the
   * error and the given detail message.
   *
  ...
  public static CreditorIdentifierFormatException forNotProperlyFormattedInput(String input) {
    return new CreditorIdentifierFormatException(
        input, String.format("'%s' format is not appropriate for a CreditorId", input));
  }

  /**
   * Creates a {@code CreditorIdentifierFormatException} telling the given Creditor Identifier check
   * digits are incorrect.
   */
  static CreditorIdentifierFormatException forIncorrectCheckDigits(String input) {
    return new CreditorIdentifierFormatException(
        input, String.format("'%s' check digits are incorrect", input));
  }

  /**
   * Creates a {@code CreditorIdentifierFormatException} telling the given Creditor Identifier
   * refers an unknown country.
   */
  static CreditorIdentifierFormatException forUnknownCountry(String input) {
    return new CreditorIdentifierFormatException(
        input, String.format("'%s' country code is not an ISO 3166-1-alpha-2 code", input));
  }

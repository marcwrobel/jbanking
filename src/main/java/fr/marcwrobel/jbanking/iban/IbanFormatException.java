...
/**
 * Thrown to indicate that an attempt has been made to convert a string to a {@link Iban}, but that
...
public final class IbanFormatException extends RuntimeException {
  ...
  /**
   * Constructs a <code>IbanFormatException</code> with the string that caused the error and the
  ...
  static IbanFormatException forNotProperlyFormattedInput(String input) {
    return new IbanFormatException(
        input, String.format("'%s' format is not appropriate for an IBAN", input));
  }
  ...
  static IbanFormatException forNotSupportedCountry(String input, IsoCountry country) {
    return new IbanFormatException(
        input, String.format("'%s' country does not support IBAN", country));
  }
  ...
  static IbanFormatException forInvalidBbanStructure(String input, BbanStructure bbanStructure) {
    return new IbanFormatException(
        input,
        String.format(
            "'%s' BBAN structure is not valid against BBAN structure used in %s",
            input, bbanStructure.getCountry()));
  }
